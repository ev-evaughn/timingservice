...
def ack(secret : str, id : int) -> None:
    try:
        now = datetime.datetime.now().strftime(strTimeF)
        sql = f'UPDATE timingserviceTimers SET ack = {now} WHERE userID = (SELECT userID FROM timingserviceUsers WHERE secret = "{secret}") AND timerID = {id}'
        db.query(sql)
    except Exception as e:
        print(f'commandHandle ack exception: {str(e)}', file=sys.stderr)
...
def soon(id : int, name : str = '', time : datetime.datetime = None, payload : object = None, address : str = '', secret : str='', ack : bool = False) -> None:
  try:
    try:
      if ack:
        try:
        ...
        if (time < tenmin):
          try:
            timeStr = time.strftime(strTimeF)
                print(json.dumps({"timerID":id, "timerName":name, "time":timeStr, "payload":payload, "address":address, "from":secret}))
            sys.stdout.flush()
          except Exception as e:
        ...
        if not res:
          try:
            ...
            if res:
              ...
              soon(res[0].get("timerID"), name, parsedTime, payload, res[0].get("address"), secret) 
              return  {
                        "type":"set alarm",
                        "payload":{
                          "status":"OK",
